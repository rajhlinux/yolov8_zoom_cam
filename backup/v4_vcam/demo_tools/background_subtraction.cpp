...
Mat resizeToFrame(const Mat& dst, const Mat& frame) 
{
    Mat resized;
    resize(dst, resized, frame.size(), 0, 0, INTER_AREA);
    return resized;
}
...
int main() 
{
    ...
    if (!video.isOpened() || !oceanVideo.isOpened()) {
    ...
    while (true) {
        ...
        if (bg.empty()) {
            oceanVideo.set(CAP_PROP_POS_FRAMES, 0);
            oceanVideo.read(bg);
        }

        bg = resizeToFrame(bg, ref_img);
